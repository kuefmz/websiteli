...
const marketKeywords = [
  ...
  "website translation Switzerland",
  "multilingual SEO Switzerland",
  "Websiteli partner",
...
function getInstagramReadMore(locale: LocaleCode) {
  const labels: Record<LocaleCode, string> = {
    ...
    fr: "À lire sur le blog de Websiteli.",
    it: "Leggilo sul blog di Websiteli.",
    ...
    ja: "Websiteliブログで詳しく読む。",
  };
  return labels[locale];
}

function getTranslation(source: BlogPostSource, locale: LocaleCode) {
  const directTranslation = source.translations[locale];
  const translation = directTranslation ?? source.translations[DEFAULT_LOCALE];
  const sourceLocale = directTranslation ? locale : DEFAULT_LOCALE;
  const isFallback = sourceLocale !== locale;
  const title = translation.title;
  const description = translation.description;
  const category = translation.category;
  const tags = Array.from(new Set([...translation.tags, ...marketKeywords]));
  const publishedAt = source.date;
  const publishDate = source.publishDate ?? source.date;
  const updatedAt = source.updated ?? publishDate;
  const excerpt = translation.excerpt;
  const summary = translation.summary?.length
    ? translation.summary
    : getDefaultSummary(source, translation);
  const keyTakeaways = translation.keyTakeaways?.length
    ? translation.keyTakeaways
    : getDefaultKeyTakeaways(translation);
  const chatGptPrompts = translation.chatGptPrompts?.length
    ? translation.chatGptPrompts
    : getDefaultChatGptPrompts(translation);
  const references = translation.references ?? [];
  const headings = getMarkdownHeadings(translation.body);
  const social = {
    linkedin: source.social?.linkedin ?? `${title}\n\n${description}`,
...
export function getBlogPosts(locale: LocaleCode = DEFAULT_LOCALE): BlogPost[] {
  return blogSources
    .filter(isPublishedSource)
    .filter((source) => sourceCanRenderLocale(source, locale))
    .map((source) => getTranslation(source, locale))
    .sort((a, b) => getPostDate(b.publishDate) - getPostDate(a.publishDate));
}

export function getBlogPost(slug: string, locale: LocaleCode = DEFAULT_LOCALE) {
  const source = blogSources.find((entry) => entry.slug === slug);
  if (!source || !isPublishedSource(source) || !sourceCanRenderLocale(source, locale)) return undefined;
  return getTranslation(source, locale);
}
